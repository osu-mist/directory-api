...
var RESPONSE_REF_REGEX = /^#\/responses\/(.+)$/;
...
exports.copy = copy;
function getAdditionalFeatures(framework) {
    var docs = [];
    for (var _i = 1; _i < arguments.length; _i++) {
        docs[_i - 1] = arguments[_i];
    }
    ...
    return additionalFeatures.filter(function (feature) {
        if (typeof feature === 'function') {
            ...
            console.warn(framework.loggingPrefix + "Ignoring " + feature + " as " + framework.featureType + " in " + additionalProperty + " array.");
            return false;
        }
    });
...
function getMethodDoc(operationHandler) {
    var doc = operationHandler.apiDoc || (Array.isArray(operationHandler) ?
...
function resolveParameterRefs(framework, parameters, definitions) {
...
function resolveResponseRefs(framework, responses, apiDoc, route) {
    return Object.keys(responses).reduce(function (resolvedResponses, responseCode) {
        ...
        if (typeof response.$ref === 'string') {
            ...
            var definition = match && (apiDoc.responses || {})[match[1]];
            if (!definition) {
